...
This document establishes the official domain glossary and ubiquitous language for Jarn (`jarn`). Both human engineers and AI coding assistants must adhere to these definitions across source code, database schemas, API contracts, tests, and documentation.
...
## Core Domain Entities (Jarn Baseline)

### Entity: Blueprint
- **Canonical Identifier**: `jarn`
- **Definition**: The foundational AI-driven software development blueprint packaging development environments, workflow rules, living specs, and quality gates.
- **Permitted Synonyms**: `jarn_blueprint`
- **Avoid**: `template_generic`, `boilerplate`, `skeleton`
...
- **Definition**: The primary multi-tenant container isolating projects, configurations, and team resources.
- **Permitted Synonyms**: None
- **Avoid**: `tenant`, `team`, `organization`, `folder`, `group`

### Entity: Project
- **Canonical Identifier**: `project_id`
- **Definition**: A concrete engineering or domain workload residing within a specific Workspace.
- **Permitted Synonyms**: None
- **Avoid**: `repo`, `job`, `task_group`, `collection`

### Entity: Artifact
- **Canonical Identifier**: `artifact_id`
- **Definition**: An immutable output, file, or bundle generated by a system process or build pipeline.
- **Permitted Synonyms**: `build_output` (in packaging context only)
- **Avoid**: `asset`, `blob`, `attachment`, `item`, `data`
